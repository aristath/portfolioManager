...
class TimeSeriesData(BaseModel):
    ...
    dates: List[str] = Field(
        ..., description="ISO format dates, e.g., ['2025-01-01', '2025-01-02']"
    )
...
class MeanVarianceRequest(BaseModel):
    ...
    symbols: List[str] = Field(
        ..., description="Ordered list of symbols matching matrix"
    )
    ...
    strategy: Literal[
        "efficient_return", "min_volatility", "efficient_risk", "max_sharpe"
    ] = Field(..., description="Optimization strategy to use")
...
class OptimizationResult(BaseModel):
    ...
    weights: Dict[str, float] = Field(
        ..., description="Optimized portfolio weights (as decimals)"
    )
    strategy_used: str = Field(..., description="Strategy that succeeded")
    constraint_level: Optional[str] = Field(
        default=None, description="Constraint level used: 'full', 'relaxed', or 'none'"
    )
    attempts: Optional[int] = Field(
        default=None, description="Number of optimization attempts"
    )
    achieved_return: Optional[float] = Field(
        default=None, description="Expected annual return achieved"
    )
    achieved_volatility: Optional[float] = Field(
        default=None, description="Expected volatility achieved"
    )
