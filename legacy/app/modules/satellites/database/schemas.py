"""Satellites module database schemas.

This module defines the schema for satellites.db which tracks:
- Bucket definitions (core and satellite portfolios)
- Virtual cash balances per bucket
...
-- Regime performance tracking (for meta-allocator - Phase 5)
...
DEFAULT_ALLOCATION_SETTINGS = [
    (
        ...
        0.00,
        "Total budget for all satellites combined (0.0-1.0)",
    ),
    ...
    (
        "reallocation_dampening",
        0.5,
        "Dampening factor for allocation changes (0.0-1.0)",
    ),
]


async def init_satellites_schema(db) -> None:
    """Initialize satellites database schema.

    Creates all tables and inserts default values.
    Also bootstraps the core bucket if it doesn't exist.
    """
    await db.executescript(SATELLITES_SCHEMA)
    ...
    if current_version == 0:
        ...
        for key, value, description in DEFAULT_ALLOCATION_SETTINGS:
            await db.execute(
        ...
        # Bootstrap core bucket
        await _bootstrap_core_bucket(db, now)

        await db.execute(
            "INSERT INTO schema_version (version, applied_at, description) VALUES (?, ?, ?)",
            (1, now, "Initial satellites schema with core bucket"),
        )
        await db.commit()
        logger.info("Satellites database schema initialized with version 1")
    else:
        await db.commit()
        logger.info("Satellites database schema initialized")


async def _bootstrap_core_bucket(db, now: str) -> None:
    """Create the core bucket if it doesn't exist."""
    existing = await db.fetchone("SELECT id FROM buckets WHERE id = 'core'")
    if not existing:
        await db.execute(
            """INSERT INTO buckets (id, name, type, status, created_at, updated_at)
               VALUES ('core', 'Core Portfolio', 'core', 'active', ?, ?)""",
            (now, now),
        )
        logger.info("Created core bucket")
