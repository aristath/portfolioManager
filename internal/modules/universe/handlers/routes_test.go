...
func TestRegisterRoutes(t *testing.T) {
	...
	testCases := []struct {
		...
		{"GET", "/securities/", "GetStocks"},
		{"GET", "/securities/US0378331005", "GetStock"},
		{"POST", "/securities/", "CreateStock"},
		{"POST", "/securities/add-by-identifier", "AddStockByIdentifier"},
		{"POST", "/securities/refresh-all", "RefreshAllScores"},
		{"POST", "/securities/US0378331005/refresh-data", "RefreshSecurityData"},
		{"POST", "/securities/US0378331005/refresh", "RefreshStockScore"},
		{"PUT", "/securities/US0378331005", "UpdateStock"},
		{"DELETE", "/securities/US0378331005", "DeleteStock"},
	}
...
func TestRegisterRoutes_RoutePrefix(t *testing.T) {
	...
	// Test that routes outside /securities prefix return 404
	req := httptest.NewRequest("GET", "/stocks", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	assert.Equal(t, http.StatusNotFound, rec.Code, "Route without /securities prefix should return 404")
}
