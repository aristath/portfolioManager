...
func TestRegisterRoutes(t *testing.T) {
	...
	testCases := []struct {
		...
		{"GET", "/charts/sparklines", "GetSparklines"},
		{"GET", "/charts/securities/US0378331005", "GetSecurityChart"},
	}
...
func TestRegisterRoutes_RoutePrefix(t *testing.T) {
	// Verify that routes are registered under /charts prefix
	service := &charts.Service{}
	...
	// Test that routes outside /charts prefix return 404
	req := httptest.NewRequest("GET", "/sparklines", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	assert.Equal(t, http.StatusNotFound, rec.Code, "Route without /charts prefix should return 404")
}
