...
func TestRegisterRoutes(t *testing.T) {
	...
	var repo *cash_flows.Repository = nil
	var depositProcessor *cash_flows.DepositProcessor = nil
	var tradernetClient cash_flows.TradernetClient = nil
	...
	testCases := []struct {
		...
		{"GET", "/cash-flows/", "GetCashFlows"},
		{"GET", "/cash-flows/sync", "SyncCashFlows"},
		{"GET", "/cash-flows/summary", "GetSummary"},
	}
...
func TestRegisterRoutes_RoutePrefix(t *testing.T) {
	// Verify that routes are registered under /cash-flows prefix
	var repo *cash_flows.Repository = nil
	var depositProcessor *cash_flows.DepositProcessor = nil
	var tradernetClient cash_flows.TradernetClient = nil
	...
	// Test that routes outside /cash-flows prefix return 404
	req := httptest.NewRequest("GET", "/sync", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	assert.Equal(t, http.StatusNotFound, rec.Code, "Route without /cash-flows prefix should return 404")
}
